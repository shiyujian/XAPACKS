...
def sailingNotice():
    ...
    for li in rep.html.find('.main_list_li'):
        ...
        if content.find('为中心') > -1 or content.find('半径') > -1:
            ...
            """ % (origin_id, link_url, title, create_time, origin_name, channelId, '0', content, radius, finalWKT)
            print('圆形区域')
        # 有多边形区域的
            ...
            if startDateTime == endDateTime:
                ...
                print('多边形')
        elif len(resultX) < 3 or len(resultY) < 3 or len(resultX) != len(resultY) or len(dayArr) < 1 or content.find('拖带') > -1:
            Sql = """
                INSERT INTO maritime_info(origin_id, link_url, title, create_time, origin_name, origin_no, type, content) 
                VALUES('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');
            """ % (origin_id, link_url, title, create_time, origin_name, channelId, '0', content)
    ...
    result = {
        ...
        'msg': 'ok',
        'content': '%s_航行通告成功新增%d条，已存在%d条，英文版%d条，未打开%d条，失败%d条' % (origin_name, codes.count(1), codes.count(2), codes.count(3), codes.count(4), codes.count(0)),
    }
    print(result)
